...
fn reports_the_file_card_facts_of_a_mixed_codec_file() {
    ...
    assert_eq!(summary.codecs, vec!["snappy".to_owned(), "zstd".to_owned()]);
    ...
    assert_eq!(summary.chunks_with_statistics, 6);
...
fn states_that_no_chunk_carries_statistics_when_the_writer_stored_none() {
    ...
    assert_eq!(reader.summary().chunks_with_statistics, 0);
...
fn describes_a_file_that_holds_no_row_group() {
    ...
    assert!(columns.columns.iter().all(|column| column.share == 0.0));
}

#[test]
fn collapses_columns_below_the_segment_limit_into_one_tail() {
    ...
    assert!(
        row.segments
            .iter()
            .all(|segment| segment.compressed_bytes >= tail.compressed_bytes / 35),
        "no returned segment is smaller than the collapsed average"
    );
...
#[test]
fn ranks_columns_by_stored_bytes_and_reports_their_share() {
    ...
    let share_sum = page.columns.iter().map(|column| column.share).sum::<f64>();
    assert!(
        (share_sum - 1.0).abs() < 1e-9,
        "shares add up to the whole file: {share_sum}"
    );
    assert!(
        page.columns[0]
            .compression_ratio
...
fn reports_chunk_facts_including_statistics_exactness() {
    ...
    assert_eq!(details.row_group_index, 0);
