...
pub struct StructureSummary {
    ...
    /// Smallest row count recorded by a row group, absent when unavailable.
    pub min_row_group_rows: Option<u64>,
    /// Largest row count recorded by a row group, absent when unavailable.
    pub max_row_group_rows: Option<u64>,
    /// Smallest stored column-chunk total recorded by a row group.
    pub min_row_group_compressed_bytes: Option<u64>,
    /// Largest stored column-chunk total recorded by a row group.
    pub max_row_group_compressed_bytes: Option<u64>,
    /// Smallest uncompressed column-chunk total recorded by a row group.
    pub min_row_group_uncompressed_bytes: Option<u64>,
    /// Largest uncompressed column-chunk total recorded by a row group.
    pub max_row_group_uncompressed_bytes: Option<u64>,
    /// Serialized footer metadata plus the eight trailing bytes.
    pub footer_bytes: u64,
    /// Distinct compression codecs in use, ordered by name, absent when incomplete.
    pub codecs: Option<Vec<String>>,
    /// Column chunks across every row group.
    pub chunk_count: u64,
    /// Column chunks that carry footer statistics.
    pub chunks_with_statistics: Option<u64>,
    /// Column chunks that carry a bloom filter.
    pub chunks_with_bloom_filter: Option<u64>,
    /// Whether whole-file and per-column chunk aggregates cover every recorded chunk.
    pub chunk_aggregates_complete: bool,
    ...
    /// Whether at least one physical column path was shortened for the wire payload.
    pub column_paths_truncated: bool,
...
pub struct StructureColumnSummary {
    /// Stable zero-based physical leaf position in the file schema.
    pub index: usize,
    /// Dotted schema path of the column.
    pub name: String,
...
impl StructureReader {
    ...
    fn summarize(
        ...
        let mut columns = schema
            ...
            .iter()
            .map(|descriptor| {
                let (name, path_truncated) = bounded_column_path(descriptor.path());
                ColumnFacts {
                    name,
                    path_truncated,
        ...
        let mut chunk_metadata_available = true;
        let mut unreadable_row_group_count = 0_usize;
        let mut first_row_offset = Some(0_u64);
        let mut min_row_group_rows = u64::MAX;
        let mut max_row_group_rows = 0_u64;
        let mut row_group_rows_available = row_group_count > 0;
        let mut min_row_group_compressed_bytes = u64::MAX;
        let mut max_row_group_compressed_bytes = 0_u64;
        let mut min_row_group_uncompressed_bytes = u64::MAX;
        let mut max_row_group_uncompressed_bytes = 0_u64;
        let mut row_group_bytes_available = row_group_count > 0;
        ...
        for (index, row_group) in metadata.row_groups().iter().enumerate() {
            ...
            if row_group.num_rows() >= 0 {
                ...
                min_row_group_rows = min_row_group_rows.min(row_group_rows);
                max_row_group_rows = max_row_group_rows.max(row_group_rows);
            } else {
                row_group_rows_available = false;
            }
            ...
            if has_layout_facts {
                ...
                uncompressed_bytes = uncompressed_bytes
                    ...
                    .ok_or(StructureError::CorruptFooter)?;
                min_row_group_compressed_bytes =
                    min_row_group_compressed_bytes.min(row_group_compressed);
                max_row_group_compressed_bytes =
                    max_row_group_compressed_bytes.max(row_group_compressed);
                min_row_group_uncompressed_bytes =
                    min_row_group_uncompressed_bytes.min(row_group_uncompressed);
                max_row_group_uncompressed_bytes =
                    max_row_group_uncompressed_bytes.max(row_group_uncompressed);
                chunks_with_bloom_filter = chunks_with_bloom_filter
                    .checked_add(row_group_blooms)
                    .ok_or(StructureError::CorruptFooter)?;
            } else {
                row_group_bytes_available = false;
                chunk_metadata_available = false;
        ...
        let column_paths_truncated = columns.iter().any(|column| column.path_truncated);
        let strings_truncated = file_metadata
            .created_by()
            .is_some_and(|value| value.len() > MAX_REPORT_FIELD_BYTES)
            || column_paths_truncated
        ...
        let summary = StructureSummary {
            ...
            min_row_group_rows: row_group_rows_available.then_some(min_row_group_rows),
            max_row_group_rows: row_group_rows_available.then_some(max_row_group_rows),
            min_row_group_compressed_bytes: row_group_bytes_available
                .then_some(min_row_group_compressed_bytes),
            max_row_group_compressed_bytes: row_group_bytes_available
                .then_some(max_row_group_compressed_bytes),
            min_row_group_uncompressed_bytes: row_group_bytes_available
                .then_some(min_row_group_uncompressed_bytes),
            max_row_group_uncompressed_bytes: row_group_bytes_available
                .then_some(max_row_group_uncompressed_bytes),
            footer_bytes,
            codecs: chunk_metadata_available.then(|| codec_names(codecs)),
            chunk_count: total_chunks,
            chunks_with_statistics: chunk_metadata_available.then_some(chunks_with_statistics),
            chunks_with_bloom_filter: chunk_metadata_available.then_some(chunks_with_bloom_filter),
            chunk_aggregates_complete: chunk_metadata_available,
        ...
        for value in [
            summary.min_row_group_rows,
            summary.max_row_group_rows,
            summary.min_row_group_compressed_bytes,
            summary.max_row_group_compressed_bytes,
            summary.min_row_group_uncompressed_bytes,
            summary.max_row_group_uncompressed_bytes,
        ]
        .into_iter()
        .flatten()
        {
    ...
    pub fn column_page(
        ...
        let (sort, direction) = if !self.summary.chunk_aggregates_complete
            && matches!(
                sort,
                StructureColumnSort::Bytes | StructureColumnSort::CompressionRatio
            ) {
            (
    ...
    pub fn report(&self, unit: StructureByteUnit) -> String {
        ...
        for (label, value) in [
            ...
            (
                ...
                if summary.chunk_aggregates_complete {
        ...
        writeln!(
            ...
            summary
                ...
                .map_or_else(|| "—".to_owned(), |codecs| codecs.join(" + "))
        )
        .expect("writing to a string cannot fail");

        if summary.codecs.is_none() {
        ...
        if self.row_groups.len() > MAX_REPORT_TABLE_ROWS {
            let suffix = summary.chunk_aggregates_complete.then(|| {
                format!(
                    " · {} bytes total",
...
mod tests {
    ...
    fn treats_a_row_group_with_negative_sizes_as_unreadable() {
        ...
        assert_eq!(reader.summary().chunk_count, 2);
        assert_eq!(reader.summary().codecs, None);
        assert_eq!(reader.summary().chunks_with_statistics, None);
        assert_eq!(reader.summary().chunks_with_bloom_filter, None);
        assert!(!reader.summary().chunk_aggregates_complete);
        ...
        assert_eq!(no_valid_chunks.summary().chunk_count, 1);
        assert!(
            no_valid_chunks
                .report(StructureByteUnit::Compressed)
                .contains("- Codecs: —")
        ...
        assert!(report.contains("- Chunks: 2"));
        assert!(report.contains("- Chunks with statistics: —"));
        assert!(report.contains("- Chunks with bloom filters: —"));
        assert!(report.contains("- Compressed chunk bytes: —"));
        assert!(report.contains("- Uncompressed chunk bytes: —"));
        assert!(report.contains("- Compression ratio: —"));
        assert!(
            report.contains("| ` value_0 ` | ` INT64 ` | — | — | — | — |"),
            "{report}"
        );
        assert!(!report.contains("- Compressed chunk bytes: 10"));
        assert!(report.contains("## Lens totals\n\nChunk metadata unavailable."));
        assert!(!report.contains("- Codec "));
    }

    #[test]
    fn incomplete_column_aggregates_never_drive_table_order() {
        let schema = test_schema_with_columns(2);
        let reader = summarize_metadata(test_metadata(vec![
            test_row_group_with_column_sizes(&schema, &[(1, 2), (100, 200)]),
            test_row_group_with_column_sizes(&schema, &[(1, 2), (-1, 200)]),
        ]))
        .expect("the partial footer remains reportable");

        assert!(!reader.summary().chunk_aggregates_complete);
        assert_eq!(reader.columns[0].compressed_bytes, 1);
        assert_eq!(reader.columns[1].compressed_bytes, 100);
    ...
    fn chunk_details_reject_every_negative_signed_fact() {
        ...
        for (label, chunk) in invalid {
            ...
            if matches!(
                ...
                assert_eq!(reader.summary().chunk_count, 1, "{label}");
                assert_eq!(reader.summary().chunks_with_statistics, None, "{label}");
                assert_eq!(reader.summary().chunks_with_bloom_filter, None, "{label}");
                assert_eq!(reader.summary().codecs, None, "{label}");
            }
        }
    }
    ...
    #[test]
    fn summary_records_exact_row_group_ranges_in_both_size_units() {
        let schema = test_schema();
        let reader = summarize_metadata(test_metadata(vec![
            test_row_group(&schema, 2, 40, 10, 30),
            test_row_group(&schema, 7, 90, 25, 80),
        ]))
        .expect("valid row-group ranges");
        let summary = reader.summary();

        assert_eq!(summary.min_row_group_rows, Some(2));
        assert_eq!(summary.max_row_group_rows, Some(7));
        assert_eq!(summary.min_row_group_compressed_bytes, Some(10));
        assert_eq!(summary.max_row_group_compressed_bytes, Some(25));
        assert_eq!(summary.min_row_group_uncompressed_bytes, Some(30));
        assert_eq!(summary.max_row_group_uncompressed_bytes, Some(80));
    }

    #[test]
    fn truncated_column_paths_keep_stable_physical_indices() {
        let prefix = "nested_".repeat(24);
        let fields = [format!("{prefix}first"), format!("{prefix}second")]
            .into_iter()
            .map(|name| {
                Arc::new(
        ...
        .expect("long paths remain bounded");
        let page = reader.column_page(
            StructureByteUnit::Compressed,
            StructureColumnSort::Index,
            StructureSortDirection::Ascending,
        ...
        assert!(reader.summary().column_paths_truncated);
        assert_eq!(page.columns[0].index, 0);
        assert_eq!(page.columns[1].index, 1);
        assert_eq!(page.columns[0].name, page.columns[1].name);
        assert!(page.columns[0].name.ends_with('…'));
        assert!(page.columns[0].name.len() <= MAX_REPORT_FIELD_BYTES);
    }
