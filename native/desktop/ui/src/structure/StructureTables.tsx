...
const INCOMPLETE_COLUMN_COLUMNS: readonly StructureGridColumn[] =
  COLUMN_COLUMNS.map((column) =>
    column.id === "name" ? column : { ...column, sortable: false },
  );
...
export function ColumnsSection({
  ...
  const effectiveSortColumnId = chunkAggregatesComplete ? sortColumnId : "name";
  const effectiveDirection =
    chunkAggregatesComplete || sortColumnId === "name"
      ? direction
      : "ascending";
    ...
    useCallback(
      (offset, limit) =>
        getStructureColumns(
          generation,
          unit,
          COLUMN_SORT_KEYS[effectiveSortColumnId] ?? "index",
          effectiveDirection,
          offset,
          limit,
        ).then((page) => ({
  ...
  const getCell = useCallback(
    (row: number, columnId: string): StructureGridCell | null => {
      ...
      switch (columnId) {
        case "name":
          return {
            text: columnPathsTruncated
              ? `#${formatNumber(column.index + 1)} · ${column.name}`
              : column.name,
            faded: false,
          };
        case "type":
        ...
        case "bytes":
          if (!chunkAggregatesComplete) {
            return { text: MISSING_FACT, faded: true };
          }
          return {
    ...
    [chunkAggregatesComplete, columnPathsTruncated, state.page, unit],
  );

  return (
    <section className="structure-card columns-card">
      <div className="structure-card-heading columns-heading">
        <div>
          <h2>Columns</h2>
          <span className="structure-card-caption">
            {chunkAggregatesComplete
      ...
      {columnPathsTruncated && (
        <p className="structure-truncation" role="status">
          Some physical column paths are shortened. Stable column numbers
          distinguish the original leaves.
        </p>
      )}
      {!ready ? (
        ...
        <StructureGrid
          label="Columns"
          columns={
            chunkAggregatesComplete ? COLUMN_COLUMNS : INCOMPLETE_COLUMN_COLUMNS
          }
          ...
          measurementPort={measurementPort}
          onSort={(columnId) => {
            if (!chunkAggregatesComplete && columnId !== "name") return;
            applySort(columnId, sortColumnId, setSortColumnId, setDirection);
          }}
          onViewportChange={requestViewport}
        />
