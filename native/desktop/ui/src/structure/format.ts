...
/** Renders available coverage as a percentage followed by its exact count. */
export function formatCoverage(present: number | null, total: number): string {
  if (present === null) return MISSING_FACT;
  const chunkLabel = total === 1 ? "chunk" : "chunks";
  if (total === 0) {
    return `${MISSING_FACT} · ${formatNumber(present)} of ${formatNumber(total)} ${chunkLabel}`;
  }
  const percent = new Intl.NumberFormat("en-US", {
    style: "percent",
    maximumFractionDigits: 0,
  }).format(present / total);
  return `${percent} · ${formatNumber(present)} of ${formatNumber(total)} ${chunkLabel}`;
}
...
export function unitLabel(unit: StructureByteUnit): string {
  return unit === "compressed" ? "On disk" : "Before compression";
}
