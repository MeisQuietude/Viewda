...
/** Renders percent-primary coverage followed by its exact present/total count. */
export function formatCoverage(present: number, total: number): string {
  if (total === 0) {
    return `${MISSING_FACT} · ${formatNumber(present)} of ${formatNumber(total)} chunks`;
  }
  const percent = new Intl.NumberFormat("en-US", {
    style: "percent",
    maximumFractionDigits: 0,
  }).format(present / total);
  return `${percent} · ${formatNumber(present)} of ${formatNumber(total)} chunks`;
}
