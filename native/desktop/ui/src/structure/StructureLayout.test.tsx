...
it("keeps layout payloads bounded and pages the minimap from the keyboard", async () => {
  ...
  const layout = vi
    ...
    .mockImplementation(async (_generation, _unit, offset) => ({
      offset,
      totalCount: 100_000,
  ...
  await waitFor(() =>
    expect(layout).toHaveBeenCalledWith(7, "compressed", 0, 80, 24),
  );
  expect(
  ...
  await waitFor(() =>
    expect(layout).toHaveBeenLastCalledWith(7, "compressed", 1, 80, 24),
  );
});
