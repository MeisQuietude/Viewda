...
describe("ColumnsSection", () => {
  ...
  it("ranks by stored bytes and states each column's facts", async () => {
    ...
    await waitFor(() =>
      expect(cellTexts("Columns by size")).toEqual([
        "column_0",
        "BYTE_ARRAY · String",
        "400.0 kB",
        "×3.0",
    ...
    expect(renderedTableWidth("Columns by size")).toBe(1_100);
  });
  ...
  it("keeps the engine's cumulative share when the view is sorted by name", async () => {
    ...
    await waitFor(() => expect(cellTexts("Columns by size")).toContain("93%"));
  });

  it("switches one mounted view without discarding table state", async () => {
    const fetchPage = vi
      .spyOn(desktop, "getStructureColumns")
      .mockResolvedValue({
        offset: 0,
    ...
    render(
      <ColumnsSection
        ...
        unit="compressed"
        columnCount={1}
        schema={<div aria-label="Test schema">Nested outline</div>}
        pinnedColumn="orders.customer.id"
        onClearPinnedColumn={onClear}
        measurementPort={measurementPort}
      />,
    );
    ...
    const sizeTab = screen.getByRole("tab", { name: "By size" });
    sizeTab.focus();
    fireEvent.keyDown(sizeTab, { key: "ArrowRight" });

    expect(screen.queryByLabelText("Columns by size")).toBeNull();
    expect(screen.getByLabelText("Test schema")).toHaveTextContent(
      "Nested outline",
    );
    ...
    const pinned = screen.getByText("Pinned in layout: orders.customer.id");
    fireEvent.click(
      within(pinned.parentElement as HTMLElement).getByRole("button", {
        name: "Clear",
      }),
    );
    expect(onClear).toHaveBeenCalledOnce();

    fireEvent.keyDown(screen.getByRole("tab", { name: "Schema" }), {
      key: "ArrowLeft",
    });
    expect(screen.getByLabelText("Columns by size")).toBeInTheDocument();
    expect(
  ...
  it("lets a one-row Structure grid pass the wheel to its outer scroller", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByLabelText("Columns by size")).toBeVisible(),
    );
    screen
      .getByTestId("structure-scroll-owner")
  ...
  it("offers no ordering for facts the engine does not rank", async () => {
    ...
    await waitFor(() =>
      expect(cellTexts("Columns by size").length).toBeGreaterThan(0),
    );
