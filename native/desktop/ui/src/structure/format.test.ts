...
describe("structure facts", () => {
  ...
  it("states coverage as a plain count of chunks", () => {
    expect(formatCoverage(96, 96)).toBe("96 of 96 chunks");
    expect(formatCoverage(0, 96)).toBe("0 of 96 chunks");
    expect(formatCoverage(1_500, 12_000)).toBe("1,500 of 12,000 chunks");
    expect(formatCoverage(0, 0)).toBe(MISSING_FACT);
  });
  ...
  it("follows the active unit when reading a row's bytes", () => {
    ...
    expect(unitLabel("uncompressed")).toBe("Uncompressed");
  });
});
